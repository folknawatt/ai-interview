<template>
  <div class="min-h-screen bg-minimal-bg text-minimal-text-primary flex flex-col items-center justify-center p-4">
    <div class="w-full max-w-4xl bg-minimal-card p-6 rounded-lg shadow-sm border border-minimal-border">
      <div class="flex justify-between items-center mb-4">
        ...
        <div class="text-2xl font-mono font-bold transition-colors" :class="isRecording ? 'text-minimal-warning' : 'text-minimal-text-primary'">
          {{ formatTime(timer) }}
        </div>
      </div>

      <div class="relative aspect-video bg-black rounded-lg overflow-hidden mb-6 border border-minimal-border">
        <video ref="videoPreview" autoplay muted playsinline class="w-full h-full object-cover"></video>

        <div v-if="!isRecording && !recordedBlob" class="absolute inset-0 flex items-center justify-center bg-black/50">
  ...
  try {
    ...
    if (selectedRole.value) {
      ...
      if (nextQ.status === 'continue') {
        ...
        // All questions completed - go to result page
        router.push('/result');
      }
    } else {
