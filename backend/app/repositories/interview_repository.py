...
class InterviewRepository(BaseRepository[InterviewSession]):
    ...
    async def get_by_session_id(self, session_id: str) -> Optional[InterviewSession]:
    ...
    async def get_question_results(self, session_id: str) -> List[QuestionResult]:
        ...
        statement = select(QuestionResult).where(
            QuestionResult.session_id == session_id)
        result = await self.session.exec(statement)
        return result.all()

    async def get_question_result(self, session_id: str, question: str) -> Optional[QuestionResult]:
    ...
    async def get_question_result_by_index(self, session_id: str, index: int) -> Optional[QuestionResult]:
        """
    ...
    async def get_aggregated_score(self, session_id: str) -> Optional[AggregatedScore]:
        """Get aggregated score for a session."""
        statement = select(AggregatedScore).where(
