...
class QuestionRepository:
    ...
    async def load_all(self, session: AsyncSession) -> dict[str, Any]:
        ...
        for role in roles:
            ...
            statement = select(Question).where(Question.role_id == role.id).order_by(Question.order)
    ...
    async def save(
        self, session: AsyncSession, role_id: str, title: str, questions: list[str]
    ) -> None:
        ...
        try:
            ...
            statement = select(Question).where(Question.role_id == role_id)
            q_result = await session.exec(statement)
            ...
            logger.info("Saved role %s with %s questions", role_id, len(questions))
        except (ValueError, RuntimeError) as e:
            await session.rollback()
            logger.error("Failed to save role %s: %s", role_id, e)
            raise RuntimeError(f"Database save failed: {e}") from e

    async def get_by_id(self, session: AsyncSession, role_id: str) -> dict[str, Any] | None:
    ...
    async def update_role(
        ...
        try:
            ...
            if questions is not None:
                ...
                q_result = await session.exec(select(Question).where(Question.role_id == role_id))
            ...
            await session.rollback()
            logger.error("Failed to update role %s: %s", role_id, e)
            raise RuntimeError(f"Database update failed: {e}") from e

    async def append_questions(
        self, session: AsyncSession, role_id: str, questions: list[str]
    ) -> None:
        ...
        try:
            ...
            q_result = await session.exec(
                select(Question).where(Question.role_id == role_id).order_by(desc(Question.order))
            )
            ...
            for idx, q_text in enumerate(questions):
                q = Question(role_id=role_id, content=q_text, order=start_order + idx)
                session.add(q)

            await session.commit()
            logger.info("Appended %s questions to role %s", len(questions), role_id)
        except ValueError:
            await session.rollback()
            raise
        except Exception as e:
            await session.rollback()
            logger.error("Failed to append questions for role %s: %s", role_id, e)
            raise RuntimeError(f"Database append failed: {e}") from e
