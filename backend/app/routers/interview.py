...
from app.dependencies import get_api_key, get_candidate_repo, get_interview_repo
from app.exceptions import NotFoundError
from app.schemas.interview import QuestionResponse, UploadPDFResponse
...
async def upload_pdf(
    ...
    result_json_str = service.generate_questions_from_pdf(
        pdf_bytes=pdf_bytes,
        num_questions=1,  # Generate 2 custom questions from resume
    )
    ...
    session_id = await InterviewService.init_session_with_questions(
        role_id, all_questions, candidate_repo, interview_repo, candidate_name, candidate_email
    )

    return UploadPDFResponse(session_id=session_id, role_id=role_id, questions=all_questions)


@router.get("/question/{role_id}/{index}")
async def get_interview_question(
    role_id: str, index: int, db: AsyncSession = Depends(get_db)
) -> QuestionResponse:
    ...
    if result.get("status") == "continue" and "question" in result:
        result["audio_path"] = await TTSService.generate_question_audio(
            text=result["question"], question_id=result["question_id"]
        )
...
async def get_session_question(
    session_id: str, index: int, skip_tts: bool = False, db: AsyncSession = Depends(get_db)
) -> QuestionResponse:
