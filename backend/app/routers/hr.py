...
async def generate_questions_api(
    data: JDInput, api_key: str = Depends(get_api_key)
) -> dict[str, Any]:
    ...
    questions = HRService.generate_questions(api_key, data.role_title, data.job_description)
    return {"suggested_questions": questions}


@router.post("/save-questions")
async def save_questions_api(
    data: SaveQuestionsRequest, session: AsyncSession = Depends(get_db)
) -> dict[str, str]:
...
@router.get("/roles")
async def get_roles(session: AsyncSession = Depends(get_db)) -> list[dict[str, Any]]:
    """Get all job roles."""
    return await HRService.get_all_roles(session)


@router.get("/roles/{role_id}")
async def get_role_details(role_id: str, session: AsyncSession = Depends(get_db)) -> dict[str, Any]:
...
async def update_role_questions(
    role_id: str, data: UpdateQuestionsRequest, session: AsyncSession = Depends(get_db)
) -> dict[str, str]:
    """Update questions for an existing role."""
    return await HRService.update_role_questions(
        session, role_id, questions=data.questions, title=data.title
    )


@router.delete("/roles/{role_id}")
async def delete_role(role_id: str, session: AsyncSession = Depends(get_db)) -> dict[str, str]:
