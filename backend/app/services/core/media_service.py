...
class MediaService:
    ...
    @staticmethod
    async def process_video_to_transcript(video_path: str) -> str | None:
        ...
        try:
            ...
            with ThreadPoolExecutor() as pool:
                ...
                audio_path = await loop.run_in_executor(pool, extract_audio, video_path)

                # Step 2: Transcribe audio to text (ASR API call)
                transcript = await loop.run_in_executor(pool, transcribe_audio, audio_path)
            return transcript
