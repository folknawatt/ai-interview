...
This module provides text-to-speech generation capabilities using multiple
providers:
...
Example:
    Generate speech using Gemini TTS:
        >>> audio_path = generate_gemini_tts(
        ...     "Hello, world!",
        ...     api_key="your-key"
        ... )
...
def generate_gemini_tts(
    ...
    Args:
        ...
        api_key (Optional[str]): Google API key. If None, uses
            GOOGLE_API_KEY from env
        voice (str): Voice name to use (default: "kore").
            Allowed voices: achernar, achird, algenib, algieba, alnilam,
            ...
            zubenelgenubi
        output_path (Optional[str]): Path to save audio file. If None,
            creates temp file
    ...
    Raises:
        TTSConfigurationError: If API key is not provided or found in
            environment
    ...
    if not api_key:
        error_msg = (
            "Google API key not provided and GOOGLE_API_KEY not found "
            "in environment"
        )
...
async def generate_tts(
    ...
    error_msg = (
        f"Invalid TTS provider: {provider}. "
        f"Supported providers: 'gemini', 'edge'"
    )
    logger.error(error_msg)
    raise TTSConfigurationError(error_msg)
